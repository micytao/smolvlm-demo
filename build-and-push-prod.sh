...
if [[ ! -f "Containerfile.web-prod" ]]; then
    echo -e "${RED}❌ Containerfile.web-prod not found${NC}"
    exit 1
fi

if [[ ! -f "prod-index.html" ]]; then
    echo -e "${RED}❌ prod-index.html not found${NC}"
    exit 1
fi

if [[ ! -f "openshift-deployment-prod.yaml" ]]; then
    echo -e "${RED}❌ openshift-deployment-prod.yaml not found${NC}"
    exit 1
fi

# Build and push production web container
echo -e "\n${PURPLE}🌐 Building Production Web Container...${NC}"
build_and_push "Containerfile.web-prod" "${WEB_PROD_IMAGE}"
...
echo "   oc apply -f rhaiis-deployment.yml"
echo ""
echo -e "${YELLOW}2. Deploy production web application:${NC}"
echo "   oc apply -f openshift-deployment-prod.yaml"
echo ""
